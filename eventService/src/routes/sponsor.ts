...
router.get('/post/partner/id', async (req, res) => {
    try {
        ...
        const response: any = await controller.getsponsorshipPostPartnerId(sponsorshipPartnerId, index);
        res.status(200).json(successResponse('getParticipantsList', response, res.statusCode));
    } catch (error) {
        console.log(error, 'error');
        res.status(500).json(errorResponse('getParticipantsList', res.statusCode));
...
router.post('/share', async (req, res) => {
    try {
        ...
        res.status(200).json(successResponse('share Sponsorship', response, res.statusCode));
    } catch (error) {
        res.status(500).json(errorResponse('error in share Sponsorship', res.statusCode));
    }
});
...
router.post('/activity', async (req, res) => {
    try {
        ...
        const controller = new SponsorshipController();
        const response: any = await controller.sponsorshipActivity(
            userId,
            sponsorshipId,
            status,
            hashtagcomment,
...
router.patch('/feadBackSponsorship', async (req, res) => {
    try {
        ...
        const controller = new SponsorshipController();
        const response: ISponsorship = await controller.feadBackSponsorship(
            body,
            sponsorshipId,
            reting,
            feadBackComment
